...
function RoomDetail() {
    ...
    // const uploadData = (collection, doc, data) => {
    //     db.collection(collection).doc(doc).add(data);
    // }
    ...
    const handleSubmitFormBooking = (event) => {
        ...
        if (formBooking.account.userName === "" || formBooking.account.email === "") {
            alert("Please Log In To Book Room")
            navigate("/login")
        }
        else if (init.length !== 0) {
            alert("This room is reserved");
        }
            ...
            if ((formBooking.in && formBooking.out && formBooking.adult && formBooking.cus) || (formBooking.in && formBooking.out && formBooking.children && formBooking.cus)) {
                event.preventDefault();
                db.collection("booking-order").doc(`${formBooking.cus + formBooking.bookDate}`).set({
                    in: formBooking.in,
                    out: formBooking.out,
